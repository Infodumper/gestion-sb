...
<body class="p-4 sm:p-10 bg-[#f0fdf4]">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-emerald-100 flex flex-col">
            <div class="px-8 py-5 border-b border-emerald-50 flex justify-between items-center bg-gray-50/50 relative">
                <div class="flex items-center gap-3">
                    <span class="text-3xl">💎</span>
                    <h1 class="brand-title text-3xl text-emerald-950 italic">CRM</h1>
                </div>
                <!-- Botón X inferior según ADN o X superior derecha según pedido usuario -->
                <button onclick="window.parent.closeAppModal()" class="w-10 h-10 bg-emerald-950 text-white rounded-full flex items-center justify-center text-xl shadow-lg transition-transform hover:scale-110">&times;</button>
            </div>
                        ...
                        <div class="subplaca-adn <?= $ya_contactado_promo ? 'contact-done' : '' ?>">
                            ...
                            <div class="subplaca-cuerpo">
                                <div class="subplaca-info">
                                    ...
                                    <p class="text-[9px] font-black text-rose-500 mt-1 uppercase tracking-wider">📅 <?= $c_dia ?> DE <?= $nombre_mes ?></p>
                                    <p class="text-[10px] text-emerald-900/30 mt-0.5"><?= s($c['Telefono']) ?></p>
                                </div>
                                <div class="subplaca-acciones !flex-row !items-center !gap-2">
                                    ...
                                    <button onclick="handleWAClick(this, '<?= get_wa_link($c['Telefono'], '¡Feliz Cumple!') ?>', <?= $c['IdCliente'] ?>, 'cumple', <?= $es_hoy ? 'true' : 'false' ?>, <?= $ya_contactado_wa ? 'true' : 'false' ?>)" 
                                    ...
                                    <button onclick="handleCheckClick(this, <?= $c['IdCliente'] ?>, 'habitual', <?= $ya_contactado_promo ? 'true' : 'false' ?>)" 
                                            class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors <?= $ya_contactado_promo ? 'bg-rose-100 text-rose-600 border-rose-200' : 'bg-white text-gray-200 border-gray-100' ?>" title="Promo Mensual">✓</button>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                        ...
                        <div class="subplaca-adn <?= $ya_promo ? 'contact-done' : '' ?>">
                            ...
                            <div class="subplaca-cuerpo">
                                <div class="subplaca-info">
                                    ...
                                    <span class="text-[8px] bg-amber-500 text-white px-2 py-0.5 rounded-full font-black mt-1 uppercase inline-block self-start"><?= $cf['CantidadCompras'] ?> COMPRAS</span>
                                    <p class="text-[10px] text-emerald-900/30 mt-0.5"><?= s($cf['Telefono']) ?></p>
                                </div>
                                <div class="subplaca-acciones !flex-row !items-center !gap-2">
                                    ...
                                    <button onclick="window.open('<?= get_wa_link($cf['Telefono'], '¡Hola!') ?>', '_blank')" class="w-8 h-8 bg-green-500 text-white rounded-full flex items-center justify-center text-lg shadow-sm hover:scale-110">💬</button>
                                    <button onclick="handleCheckClick(this, <?= $cf['IdCliente'] ?>, 'habitual', <?= $ya_promo ? 'true' : 'false' ?>)" 
                                            class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors <?= $ya_promo ? 'bg-amber-100 text-amber-600 border-amber-200' : 'bg-white text-gray-200 border-gray-100' ?>">✓</button>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
    ...
    <script>
        function handleWAClick(btn, url, id, tipo, esHoy, yaEnviado) {
            if (!esHoy) { Swal.fire({ icon: 'info', title: 'Aún no es su cumpleaños', text: 'El saludo solo puede enviarse el mismo día.', confirmButtonText: 'Aceptar', confirmButtonColor: '#00a876' }); return; }
            if (yaEnviado || btn.classList.contains('btn-wa-disabled')) { Swal.fire({ icon: 'warning', title: 'Saludo ya enviado', text: 'Ya has enviado la felicitación hoy.', confirmButtonText: 'Aceptar', confirmButtonColor: '#00a876' }); return; }
            window.open(url, '_blank');
            btn.classList.add('btn-wa-disabled');
            const fd = new FormData(); fd.append('id', id); fd.append('tipo', tipo);
            fetch('ajax_mark_contacted.php', { method: 'POST', body: fd });
        }

        async function handleCheckClick(btn, id, tipo, yaEnviado) {
            const isDone = btn.classList.contains('bg-rose-100') || btn.classList.contains('bg-amber-100');
            if (isDone) { Swal.fire({ icon: 'info', title: 'Promo ya enviada', text: 'Ya has marcado este cliente este mes.', confirmButtonText: 'Aceptar', confirmButtonColor: '#00a876' }); return; }
            const col = (tipo==='cumple')?'rose':'amber';
            btn.classList.add('bg-'+col+'-100', 'text-'+col+'-600', 'border-'+col+'-200');
            btn.closest('.subplaca-adn').classList.add('contact-done');
            const fd = new FormData(); fd.append('id', id); fd.append('tipo', tipo);
            await fetch('ajax_mark_contacted.php', { method: 'POST', body: fd });
            Swal.fire({ icon: 'success', title: 'Marcado', text: 'Contacto registrado.', confirmButtonText: 'Aceptar', confirmButtonColor: '#00a876' });
        }
